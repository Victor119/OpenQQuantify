...
class ultra_low_frequency(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, 
                ...
                carrier_freq,
                mod_freq,
                mod_index_range=(0, 1, 0.1, 0.5, 200), 
                mod_freq_range=(300, 2700, 100, 1000, 200), 
                carrier_freq_range=(300, 2700, 0.1, 1000, 200)
                ):
        ...
        self.settings = Qt.QSettings("GNU Radio", "ultra_low_frequency")
        ...
        self._mod_index_range = qtgui.Range(*mod_index_range)
        self._mod_index_win = qtgui.RangeWidget(self._mod_index_range, self.set_mod_index, "Modulation Index", "counter", float, QtCore.Qt.Horizontal)
        self.top_layout.addWidget(self._mod_index_win)
        self._mod_freq_range = qtgui.Range(*mod_freq_range)
        self._mod_freq_win = qtgui.RangeWidget(self._mod_freq_range, self.set_mod_freq, "Modulation Frequency", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_layout.addWidget(self._mod_freq_win)
        self._carrier_freq_range = qtgui.Range(*carrier_freq_range)
        self._carrier_freq_win = qtgui.RangeWidget(self._carrier_freq_range, self.set_carrier_freq, "Carrier Freq", "counter_slider", float, QtCore.Qt.Horizontal)
        ...
        self.analog_sig_source_x_0 = analog.sig_source_f(64000, analog.GR_COS_WAVE, mod_freq, 1, 0, 0)
...
def generate_am_signal(sample_rate, carrier_freq, modulation_index, modulation_frq, duration, output_dir):
    qapp = Qt.QApplication(sys.argv)
    
    tb = ultra_low_frequency(
        samp_rate=sample_rate,
        mod_index=modulation_index,
        carrier_freq=carrier_freq,
        mod_freq = modulation_frq
    )
    
    # Create a file_sink block
    output_dir_path = os.path.join(output_dir, f"am_signal_{carrier_freq}_{modulation_index}_{modulation_frq}_{sample_rate}.dat")
    file_sink = blocks.file_sink(gr.sizeof_float, output_dir_path)
    ...
    Qt.QTimer.singleShot(duration, lambda: qapp.quit())
...
def main():
    #path to the directory train from directory dataset
    data_path = 'C:\\Users\\victor\\Documents\\OpenQuantify_unpaid_25_11_2024\\week_3\\dataset\\train\\ULF_AM'
    
    carrier_frequencies = [300, 2700]
    modulation_indices = [0.300, 0.500, 0.700]
    modulation_freq = [1000]
    samp_rate = 64000

    for freq in carrier_frequencies:
        for mod_idx in modulation_indices:
            for mod_frq in modulation_freq:
                generate_am_signal(
                    sample_rate = samp_rate,
                    carrier_freq = freq,
                    modulation_index = mod_idx,
                    modulation_frq = mod_frq,
                    duration = 1000,
